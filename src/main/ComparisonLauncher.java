...
public class ComparisonLauncher {
  ...
  public ComparisonLauncher(
      CardinalityEstimationAlgorithm alg,
      int size,
      int m,
      int[] cardinalities,
      double alpha,
  ...
  public double[] getAvgMCEstimatesVaryM() {
    ...
    for (int i = 0; i < t; i++) {
      for (int j = 0; j < m; j++) {
        relErrors[i][j] =
            (Math.abs(varyMMCEstimates[i][j] - cardinalities[bigN - 1])) / cardinalities[bigN - 1];
      }
    }
    // return the average of the trials for each element. This is a 1D array
  ...
  public double[] getAvgPCEstimatesVaryM() {
    ...
    for (int i = 0; i < t; i++) {
      for (int j = 0; j < m; j++) {
        relErrors[i][j] =
            (Math.abs(varyMPCEstimates[i][j] - cardinalities[bigN - 1])) / cardinalities[bigN - 1];
      }
    }
    // return the average of the trials for each element. This is a 1D array
    return averageOverTrials(relErrors);
  ...
  public double[] getAvgHBBEstimatesVaryM() {
    ...
    for (int i = 0; i < t; i++) {
      for (int j = 0; j < m; j++) {
        relErrors[i][j] =
            (Math.abs(varyMHBBEstimates[i][j] - cardinalities[bigN - 1])) / cardinalities[bigN - 1];
      }
    }
    // return the average of the trials for each element. This is a 1D array
    return averageOverTrials(relErrors);
  ...
  public double[] getAvgNewAlgEstimatesVaryM() {
    ...
    for (int i = 0; i < t; i++) {
      for (int j = 0; j < m; j++) {
        relErrors[i][j] =
            (Math.abs(varyMNewAlgEstimates[i][j] - cardinalities[bigN - 1]))
                / cardinalities[bigN - 1];
      }
    }
    // return the average of the trials for each element. This is a 1D array
    return averageOverTrials(relErrors);
  ...
  public static void main(String[] args) throws FileNotFoundException {
    ...
    boolean isTest = Boolean.parseBoolean(args[1]);
    ...
    CardinalityEstimationAlgorithm alg = new algs.ProbabilisticCounting(m, phi);

    ComparisonLauncher launcher =
        new ComparisonLauncher(alg, size, m, cardinalities, alpha, trials, input);

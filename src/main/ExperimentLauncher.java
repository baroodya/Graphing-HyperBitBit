...
public class ExperimentLauncher {
  ...
  // Every estimate collected in all of the trials for each value of m (only the final value of each
  // trial)
  ...
  public ExperimentLauncher(
      String alg, int size, int m, int[] cardinalities, double alpha, double phi, int trials)
      throws FileNotFoundException {
    // Save the constants to be used in other methods
    bigN = size;
    t = trials;
  ...
  public double[] getAvgNormalizedEstimates() {
    ...
    for (int i = 0; i < bigN; i++) returnThis[i] = (regEstimates[i] / cardinalities[i]) - 1;
  ...
  public double[][] getAllRelativeErrorsVaryM() {
    ...
    for (int i = 0; i < t; i++) {
      for (int j = 0; j < m; j++) {
        varyMRelErrors[i][j] = (Math.abs(varyMEstimates[i][j] - n)) / n;
      }
    }
  ...
  protected static void readInputs(boolean isTest) throws IOException {
    ...
    System.out.print(
        "Hello! Welcome to the Cardinality Estimation Algorithm Experiment Launcher! ");
    ...
    if (isTest) {
      ...
      createOutput(alg, file, false, maxRead, m, alpha, phi, trials, numberOfTrialsShown, true);
      synthetic = true;
      createOutput(alg, file, true, maxRead, m, alpha, phi, trials, numberOfTrialsShown, true);
    }
    ...
    if (newValues) {
      ...
      s = keyboardInput.nextLine();
      if (!s.equals("")) maxRead = Integer.parseInt(s);
      ...
      s = keyboardInput.nextLine();
      if (!s.equals("")) m = Integer.parseInt(s);
      System.out.print("Enter Trials (T): ");
      s = keyboardInput.nextLine();
      if (!s.equals("")) trials = Integer.parseInt(s);
      System.out.print("Enter 𝛼: ");
      s = keyboardInput.nextLine();
      if (!s.equals("")) alpha = Double.parseDouble(s);
      System.out.print("Enter \uD835\uDF11: ");
      s = keyboardInput.nextLine();
      if (!s.equals("")) phi = Double.parseDouble(s);
      System.out.print("Enter Number of Trials Shown: ");
      s = keyboardInput.nextLine();
      if (!s.equals("")) numberOfTrialsShown = Integer.parseInt(s);
    }
  ...
  private static void runExperiments(
    ...
    System.out.println(
        TimingTracker.timing(alg, "'" + input + "'", m, trials, "src/timings.txt") + "\n");

    ExperimentLauncher launcher;
    if (synthetic)
      launcher = new ExperimentLauncher(alg, size, m, cardinalities, alpha, phi, trials);
    else launcher = new ExperimentLauncher(alg, size, m, cardinalities, alpha, phi, trials, input);
    ...
    System.out.println(
        "\nThis experiment took "
            + TimingTracker.add(
                alg, "'" + input + "'", m, trials, (endTime - startTime), "src/timings.txt"));
  }

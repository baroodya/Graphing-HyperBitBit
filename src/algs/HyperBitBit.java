...
public class HyperBitBit implements CardinalityEstimationAlgorithm {
  ...
  protected void count(String s) {
    ...
    if (Bits.r(hasher.hash(s)) > avg + 1) sketch2 = sketch2 | (1L << hasher.hash2(s, m));
  ...
  public static void main(String[] args) throws FileNotFoundException {
    ...
    if (synthetic) for (int i = 0; i < N; i++) counter.readSyntheticElement(Math.random());
    else for (String x : stream) counter.readElement(x);

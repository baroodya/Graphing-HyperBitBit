...
class CustomGrapherTest {
    ...
    CustomGrapherTest() {
        ...
        for (int i = 0; i < 10; i++)
            averageArrayOfSomeOtherArray_someThirdArray[i] = (someOtherArray_Size10[i] + someThirdArray_Size10[i]) / 2;
        averageColOfSomeOtherArray_someThirdArray = DoubleColumn.create("averageColOfSomeOtherArray_someThirdArray", averageArrayOfSomeOtherArray_someThirdArray);
        ...
        someTable_Size3X10 = Table.create("someTable_Size3X10", someColumn_Size10, someOtherColumn_Size10, someThirdColumn_Size10);
        someTable_XValues_Avg_2X10 = Table.create("someTable_XValues_Avg_2X10", someColumn_Size10, averageColOfSomeOtherArray_someThirdArray, someOtherColumn_Size10, someThirdColumn_Size10);
        ...
        lineTraceOfSomeArray_SomeOtherArray = ScatterTrace.builder(someColumn_Size10, someOtherColumn_Size10).mode(ScatterTrace.Mode.LINE).showLegend(true).line(grey).name("someOtherArray_Size10").build();
        scatterTraceOfSomeArray_SomeOtherArray = ScatterTrace.builder(someColumn_Size10, someOtherColumn_Size10).mode(ScatterTrace.Mode.MARKERS).showLegend(true).line(grey).name("someOtherArray_Size10").build();

        lineTraceOfSomeOtherArray_SomeThirdArray = ScatterTrace.builder(someOtherColumn_Size10, someThirdColumn_Size10).mode(ScatterTrace.Mode.LINE).showLegend(false).line(grey).opacity(0.25).build();
        scatterTraceOfSomeOtherArray_SomeThirdArray = ScatterTrace.builder(someOtherColumn_Size10, someThirdColumn_Size10).mode(ScatterTrace.Mode.MARKERS).showLegend(false).line(grey).opacity(0.25).build();
        ...
        someLayout = Layout.builder().title(title).height(600).width(800).xAxis(someXAxis).yAxis(someYAxis).build();

        n = 0;
        maxRange = 1;
        histogramXAxis = Axis.builder().fixedRange(true).range(n - maxRange, n + maxRange).build();
        histogramLayout = Layout.builder().title(title).height(600).width(800).xAxis(histogramXAxis).build();
        ...
        String[] distributionHeaders = new String[2];
        lineDistributionTraces64 = new Trace[3];
        lineDistributionTraces16 = new Trace[3];
        scatterDistributionTraces64 = new Trace[3];
        scatterDistributionTraces16 = new Trace[3];

        for (int i = 0; i < 10; i++) {
            distributionXValues[i] = ((i + 1) * (0.1) + (0.05));
            heights_SomeOtherArray_SomeThirdArray[0][(int) Math.floor(someOtherArray_Size10[i] * 10)]++;
        }

        for (int i = 0; i < 10; i++)
            heights_SomeOtherArray_SomeThirdArray[1][(int) Math.floor(someThirdArray_Size10[i] * 10)]++;

        distributionHeaders[0] = "m = 2";
        distributionHeaders[1] = "m = 8";
        distributionTable64 = grapher.createTable(title, title, distributionXValues, heights_SomeOtherArray_SomeThirdArray, distributionHeaders);
        distributionHeaders[1] = "m = 4";
        distributionTable16 = grapher.createTable(title, title, distributionXValues, heights_SomeOtherArray_SomeThirdArray, distributionHeaders);


        for (int i = 0; i < lineDistributionTraces64.length - 1; i++) {
            switch (i % 3) {
                case 0:
                    line = purple;
                    break;
        ...
        lineDistributionTraces64[lineDistributionTraces64.length - 1] = ScatterTrace.builder(dashColX, dashColY).mode(ScatterTrace.Mode.LINE).line(dashed).name("n = " + cardinality).build();
        scatterDistributionTraces64[scatterDistributionTraces64.length - 1] = ScatterTrace.builder(dashColX, dashColY).mode(ScatterTrace.Mode.LINE).line(dashed).name("n = " + cardinality).build();
        lineDistributionTraces16[lineDistributionTraces16.length - 1] = ScatterTrace.builder(dashColX, dashColY).mode(ScatterTrace.Mode.LINE).line(dashed).name("n = " + cardinality).build();
        scatterDistributionTraces16[scatterDistributionTraces16.length - 1] = ScatterTrace.builder(dashColX, dashColY).mode(ScatterTrace.Mode.LINE).line(dashed).name("n = " + cardinality).build();
    ...
    void createTable() {
        ...
        output = grapher.createTable("someTable_Size3X10", "someArray_Size10", someArray_Size10, someOtherArray_someThirdArray, someHeaders);
        assert output.toString().equals(someTable_Size3X10.toString());

        output = grapher.createTable("someTable_XValues_Avg_2X10", "someArray_Size10", someArray_Size10, averageArrayOfSomeOtherArray_someThirdArray, someOtherArray_someThirdArray);
    ...
    void showScatterDistributions() {
        ...
        grapher.showScatterDistributions(title, someOtherArray_someThirdArray, 10, 16, cardinality);
